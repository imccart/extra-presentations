...
.pull-left[
loss of CS due to bunching?
]

.pull-right[
gain to total welfare with new scoring design
]
...
Two-step process:
1. Estimate mean preferences, $\hat{\delta}_{j}$, from $u_{ij} = \color{red}{\delta_{j}} + \xi_{j} + \varepsilon_{ij}$, 
...
- Paper shows that any monotone partial score reduces to two elements: an aggregator and a cutoff function
...
- No firm response to quality bonuses, but costs should be in $\rho^{G}$
...
.pull-left[
  ![:scale 57%](images/best_linear_substitute_aggregator_paper.pdf)
]

.pull-right[
  ![:scale 57%](images/best_linear_replacement_cutoff_placement_paper.pdf)
]
...
.center[
  ![:scale 57%](images/best_substitute_eqm_unpacking_custom_levels_0.pdf)
]
...
.center[
  ![:scale 57%](images/best_substitute_eqm_unpacking_custom_levels_1.pdf)
]
...
.center[
  ![:scale 57%](images/best_substitute_eqm_unpacking_custom_levels_2.pdf)
]
...
.center[
  ![:scale 57%](images/best_substitute_eqm_unpacking_custom_levels_3.pdf)
]
...
.center[
  ![:scale 57%](images/best_substitute_eqm_unpacking_custom_levels_4.pdf)
]
...
- Alert for ratings below 3 stars? **Maybe**
